...
Enum.each(0..num_users, fn _n ->
  %User{
    ...
    company_id: Repo.one(from c in Company, order_by: fragment("RANDOM()"), select: c.id, limit: 1),
  } |> Repo.insert! end)
  ...
  Enum.each(0..num_transactions, fn _n ->
    ...
    %Transaction{
      ...
      company_id: Repo.one(from c in Company, order_by: fragment("RANDOM()"), select: c.id, limit: 1),
      merchant_id: Repo.one(from m in Merchant, order_by: fragment("RANDOM()"), select: m.id, limit: 1),
